...
export function fitBoard(width: number, height: number, graph: boolean, largeText: boolean, total = 200, textScale = largeText ? 1 : 0) {
  ...
  const scale = Math.max(0, Math.min(4, textScale));
  const minimumRow = graph ? [48, 52, 62, 74, 86][scale] : [27, 30, 38, 46, 54][scale];
  ...
  return { columns, rows, capacity: rows * columns, rowHeight: graph ? Math.min([52, 56, 66, 78, 90][scale], rowHeight) : rowHeight };
}
