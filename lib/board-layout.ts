...
export function fitBoard(width: number, height: number, graph: boolean, largeText: boolean, total = 200, textScale = largeText ? 1 : 0) {
  ...
  const minimumRow = graph ? (textScale === 2 ? 62 : largeText ? 52 : 48) : textScale === 2 ? 38 : largeText ? 30 : 27;
  ...
  return { columns, rows, capacity: rows * columns, rowHeight: graph ? Math.min(textScale === 2 ? 66 : 56, rowHeight) : rowHeight };
}
