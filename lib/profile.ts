...
export type WatchlistId = 0 | 1 | 2 | 3 | 4;
export type Watchlists = [StockSelection[], StockSelection[], StockSelection[], StockSelection[], StockSelection[]];
...
const validList = (value: unknown): value is WatchlistId => value === undefined || value === 0 || value === 1 || value === 2 || value === 3 || value === 4;
export function profileWatchlists(profile: Profile): Watchlists {
  const lists: unknown = profile.watchlists;
  // 이전 3개/4개 관심목록 프로필은 그대로 보존하고,
  // 새 5번째 숏관심 목록만 빈 배열로 추가한다.
  if (Array.isArray(lists) && lists.length === 5) return lists as Watchlists;
  if (Array.isArray(lists) && lists.length === 4) return [
    lists[0] as StockSelection[],
    lists[1] as StockSelection[],
    lists[2] as StockSelection[],
    lists[3] as StockSelection[],
    [],
  ];
  if (Array.isArray(lists) && lists.length === 3) return [
    lists[0] as StockSelection[],
    lists[1] as StockSelection[],
    lists[2] as StockSelection[],
    [],
...
export function parseOperation(value: unknown): ProfileOperation {
  ...
  if (op.type === 'import' && Array.isArray(op.watchlist) && op.watchlist.length <= 200 && Array.isArray(op.highlights) && op.highlights.length <= 2000) {
    const receivedLists = Array.isArray(op.watchlists) && (op.watchlists.length === 3 || op.watchlists.length === 4 || op.watchlists.length === 5) ? op.watchlists : [op.watchlist, [], [], [], []];
    const rawLists = [...receivedLists, ...Array.from({ length: 5 - receivedLists.length }, () => [])] as Watchlists;
...
export function restoreProfile(value: unknown): Profile | null {
  ...
  const receivedLists = Array.isArray(storedLists) && (storedLists.length === 3 || storedLists.length === 4 || storedLists.length === 5) ? storedLists : [profile.watchlist, [], [], [], []];
  const rawLists = [...receivedLists, ...Array.from({ length: 5 - receivedLists.length }, () => [])] as Watchlists;
