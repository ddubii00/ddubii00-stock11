    ...
    <Dialog open={open} onOpenChange={(value) => { setOpen(value); if (!value) { setPassword(''); setError(''); } }}>
      <DialogContent>
        ...
        {sync.phase === 'ready' ? <div className="profile-account">
          ...
          {sync.profile.revision === 0 && !sync.unsaved && ((localImport.watchlists?.some((list) => list.length > 0) ?? localImport.watchlist.length > 0) || localImport.highlights.length > 0) && <Button onClick={() => sync.send(localImport)}>이 브라우저 기록 가져오기</Button>}
