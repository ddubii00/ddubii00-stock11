...
추가 UI 요구도 반영했습니다. 모든 시장·관심종목의 한 번 클릭은 투명 노랑, 더블클릭은 투명 빨강, 표시된 배경을 다시 한 번 클릭하면 해제됩니다. 같은 종목의 색상은 시장·관심종목·차트에서 공유합니다. 관심종목의 `=` 손잡이는 제거하고 셀 배경 자체를 드래그해 순서를 바꿉니다. NASDAQ 차트 바로 다음에 Dow / Dow 차트를 추가했습니다(Dow 구성 30종목).
