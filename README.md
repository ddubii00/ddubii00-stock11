...
- 가격표 최대 4열, 넓은 화면에서 차트 약 52~64종목, 태블릿 방향별 2~3열. 차트 셀은 기본 최소 60px, 추세 영역은 40px로 압축하고 가격 글자 크기는 유지합니다. 모든 200종목은 하단 페이지로 이동합니다.
...
종목 차트는 시가총액 순위대로 위에서 아래로 배치한 후 다음 열로 이어집니다. 종목명만 네이버 PC 페이지로 연결되며, 셀 배경을 누르면 반투명 노란색 표시를 켜거나 끕니다. 순위 버튼(차트는 배경 버튼)을 키보드로 조작할 수도 있습니다. 표시는 시세 갱신과 탭 전환 중 유지되며 페이지 새로고침 시 초기화됩니다. 헤더 및 종목 차트의 가로 영역은 숫자의 자릿수와 무관하게 유지됩니다.
