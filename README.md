...
`관심종목` 또는 `관심종목 차트`에서 종목명/코드를 입력합니다. `삼성`, `애플`, `AAPL`, `IBM` 등으로 한국·미국 주식을 검색하며 마우스 또는 ↑↓ / Enter로 추가합니다. 중복은 방지하고 최대 200개까지 이 브라우저의 localStorage에 저장합니다. 각 종목 셀 오른쪽 위의 작은 ×로 삭제하며 다른 기기와 자동 동기화하지 않습니다. 저장 실패와 검색 실패는 화면에 표시합니다. 수신 대기 종목도 이름과 삭제 버튼을 유지합니다.
...
관심종목의 셀 배경은 **한 번 클릭하면 반투명 노란색**, **더블클릭하면 반투명 빨간색**으로 표시합니다. 노란색에서 다시 한 번 클릭하거나 빨간색에서 다시 더블클릭하면 표시를 해제합니다. 종목명 링크와 삭제·순서 이동 버튼은 배경색을 바꾸지 않습니다. × 아래의 **= 손잡이**를 다른 종목 셀로 드래그해 순서를 바꿉니다. 키보드로 손잡이에 초점을 두면 ↑↓로 한 칸씩, Home/End로 처음/끝으로 옮길 수 있습니다. 변경한 순서는 두 관심종목 탭에서 공유하며 브라우저에 저장되어 새로고침 후에도 유지됩니다.
...
시장별 종목 차트는 시가총액 순위대로 위에서 아래로 배치한 후 다음 열로 이어집니다. 종목명만 네이버 PC 페이지로 연결되며, 일반 시장 셀 배경을 한 번 누르면 표시를 켜거나 끕니다(새 표시는 빨간색). 관심종목에서 지정한 노란색·빨간색은 같은 종목의 시장별 시세·차트에도 적용되며 해제도 양방향으로 공유합니다. 순위 버튼(차트는 배경 버튼)을 키보드로 조작할 수도 있습니다. 색상 표시는 시세 갱신과 탭 전환 중 유지되며 페이지 새로고침 시 초기화됩니다. 헤더 및 종목 차트의 가로 영역은 숫자의 자릿수와 무관하게 유지됩니다.
