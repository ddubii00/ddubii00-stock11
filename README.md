...
KOSPI · KOSDAQ · NASDAQ 시가총액 상위 100종목을 보는 Next.js 대시보드입니다.

- 밝은 테마, 6개 시세/분봉 탭, 주요 지수와 장중/장종료 표시
- 가격표 최대 4열·100종목, 큰 화면에서 분봉 28~32종목, 태블릿 방향별 2~3열
...
KIS의 국내 `H0STCNT0`, NASDAQ `HDFSCNT0` 시세를 한 개의 공유 WebSocket으로 받고 `/api/live` SSE를 통해 화면에 전달합니다. 새로운 체결은 현재가·등락률·해당 분의 그래프에 반영합니다. 정규장 시간 외 체결은 제외하고 장종료 후에는 네이버 정규장 최종가격을 유지합니다.

- 기본 동시 구독 상한은 **40종목**입니다. 여러 접속자도 이 한도를 공유합니다. 화면 앞 순위부터 구독하고, 나머지 종목과 지수는 30초 네이버 갱신을 유지합니다. **100종목 모두 KIS 실시간이라고 표시하지 않습니다.** 화면 하단에 승인된 구독 수를 표시합니다.
