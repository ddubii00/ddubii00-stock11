...
KOSPI · KOSDAQ · NASDAQ · S&P500 구성 종목의 시가총액 상위 200종목과 Dow 30종목을 보는 Next.js 대시보드입니다.

- 밝은 테마, 시장별 시세/차트와 관심종목을 포함한 12개 탭, 주요 지수와 장중/장종료 표시
...
`관심종목` 또는 `관심종목 차트`에서 종목명/코드를 입력합니다. `삼성`, `애플`, `AAPL`, `IBM` 등으로 한국·미국 주식을 검색하며 마우스 또는 ↑↓ / Enter로 추가합니다. 중복은 방지하고 최대 200개까지 저장합니다. 각 종목 셀 오른쪽 위의 작은 ×로 삭제합니다. Redis와 비밀번호를 설정하면 로그인 후 관심종목·순서·색상·글자 크기 설정을 기기 간 동기화합니다. 미설정 시 기존 브라우저 localStorage 모드를 유지합니다. 저장 실패와 검색 실패는 화면에 표시합니다. 수신 대기 종목도 이름과 삭제 버튼을 유지합니다.
...
모든 시장과 관심종목의 셀 배경은 **한 번 클릭하면 반투명 노란색**, **더블클릭하면 반투명 빨간색**, **색상이 있는 상태에서 한 번 더 클릭하면 해제**됩니다. 종목명 링크와 삭제·순서 이동 버튼은 배경색을 바꾸지 않습니다. × 아래의 **= 손잡이**를 다른 종목 셀로 드래그해 순서를 바꿉니다. 키보드로 손잡이에 초점을 두면 ↑↓로 한 칸씩, Home/End로 처음/끝으로 옮길 수 있습니다. 변경한 순서는 두 관심종목 탭에서 공유하며 저장 모드에 따라 Redis 또는 브라우저에 저장되어 새로고침 후에도 유지됩니다.
...
시장별 종목 차트는 시가총액 순위대로 위에서 아래로 배치한 후 다음 열로 이어집니다. 종목명만 네이버 PC 페이지로 연결됩니다. 관심종목과 같은 종목의 시장별 시세·차트는 색상과 해제를 양방향 공유합니다. 순위 버튼(차트는 배경 버튼)을 키보드로 조작할 수도 있습니다. 로컬 모드에서는 같은 주소의 다른 탭에도 변경·해제가 반영되지만 다른 기기와는 독립적입니다. 서버 모드에서는 동일 Redis DB·prefix·사용자 ID를 사용하는 Vercel과 Oracle이 설정을 공유합니다. 사이트 데이터를 지워도 Redis의 저장 내용은 유지됩니다. 헤더 및 종목 차트의 가로 영역은 숫자의 자릿수와 무관하게 유지됩니다.

## Redis Cloud 영구 저장 및 로그인

표준 `redis`(node-redis)와 서버 환경변수 `REDIS_URL`로 접속합니다. 비밀번호 로그인에는 `STOCK11_SYNC_PASSWORD`(12~256자)가 추가로 필요하며, 키와 비밀번호를 채팅이나 Git에 올리지 마세요. Vercel 전용 KV/REST API를 사용하지 않습니다. 기존 시세·KIS 경로와 분리되어 저장소 장애가 시장정보 조회를 중단시키지 않습니다.
