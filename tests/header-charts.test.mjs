...
test('header returns all five charts and keeps the last FX quote in each minute without filling gaps', async () => {
  ...
  globalThis.fetch = async (url) => {
    ...
    if (url.includes('FX_USDKRW')) return Response.json({ result: {
      tradeBaseAt: '20260908', localDateTimeNow: '20260908100000', lastClosePrice: 1347,
      priceInfos: [
        ...
        { localDateTime: '20260908090010', currentPrice: 1342.6 },
        { localDateTime: '20260908090047', currentPrice: 1343 },
        { localDateTime: '20260908100000', currentPrice: 1345.5 },
        { localDateTime: '20260908100100', currentPrice: 1400 },
  ...
  try {
    ...
    assert.deepEqual(series['USD/KRW'].points, [{ minute: 540, price: 1343 }, { minute: 600, price: 1345.5 }]);
    assert.equal(series['USD/KRW'].previousClose, 1347);
    assert.equal(series['USD/KRW'].session.timeZone, 'Asia/Seoul');
    assert.equal(series['USD/KRW'].session.start, 540);
    assert.equal(series['USD/KRW'].session.end, 930);
