...
export async function GET(request: Request) {
  ...
  // KOSPI/KOSDAQ prices use the relay's KIS multi REST path. Keep this SSE
  // proxy for overseas compatibility only; domestic subscriptions must not
  // compete for the shared WebSocket limit.
  if (!['NASDAQ', 'NYSE', 'AMEX'].includes(market ?? '') || !codes.length || codes.length > 200 || codes.some((code) => !/^[A-Za-z0-9.^-]{1,24}$/.test(code))) {
